...
<%
   ...
   /* if (loginUser.getFlag()==25 && loginUser.getZzsj().after(sdate)){
        if (p!=null && p.getFlag()==30){
        	out.println("<script language='javascript'>{window.alert('您的VIP权限还没到期，请联系管理员办理VIP会员升级白金VIP会员事宜！');history.go(-1);}</script>");
           	return;
            
        	 /*zxl delete VIP升级白金VIP时要联系管理员补差价
            ...
            if (p.getDatetype()==1){
                ...
                if (sdate.before(loginUser.getZzsj())){
                    out.println("<script language='javascript'>{window.alert('请选择大于您VIP剩余期限的白金VIP会员项目');}</script>");
                    return;
                }
...
<form id="weixin" action="weixinpay/wxpayapi.jsp" name="form1" method="post">
    ...
    <input type="hidden" name="body" value="<%=p.getName()%>"/>
 	...
 	<%
    if("18205167136".equalsIgnoreCase(loginUser.getUsername())){
    	%>
