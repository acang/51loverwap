var Validate = Exp.Class.extend("Validate", {
	init: function(){
		this.start();
	},
	start:function(){
		...
		if($){
			...
			* @param {String} opts 统一的配置
			* @param {Boolean} hide 验证名称
			*/
					...
					if(validate){
						...
						for(var name in cache){
							if(this[0] == cache[name].element[0]){
								return form.valide(name || this.attr("name"));
							}
						}
					}
				}
			...
			(function(i){
				...
				if(newObj.method === 'blur'){
					element.blur(function(){
						self.handleValide(newObj, 'blur', false);
					});
				}
			})(i);
		}
	...
	* @param {Boolean} ignoreNotShow 是否忽略不显示
	*/
	handleValide: function(opt, eventName, ignoreNotShow){
		...
		if(eventName == "init"){
			...
			this.showValide(opt, el.attr("bgvalide"), hide, ignoreNotShow);
		...
		for(var i=0,l=regs.length; i<l; i++){
			...
			if(reg && reg.test && !reg.test(val)){
				var hide = opt.proxyCallback(true, eventName, val);
				this.showValide(opt, msgs[i], hide, ignoreNotShow);
		...
		if(i == l){
			...
			if(opt.condition){
				ret = opt.condition(val, opt);
				if(opt.type == "error" && ret){
					var hide = opt.proxyCallback(true, eventName, val);
					this.showValide(opt, ret === true || ret == "true" ? "" : ret, hide, ignoreNotShow);
			...
			if(remote && remote.checkEvent.indexOf(eventName) != -1){
				...
				var ajaxOpt = {
					...
					data: remote.data,
					async: eventName == "submit" ? false : true,
					error:function(){
					},
					success: function(data){
						ret = remote.condition? remote.condition.call(opt, data) : data;;
						msg = ret === true ? "" : ret;
						if(opt.type == "error"){
							if(ret){
								var hide = opt.proxyCallback(ret, eventName, val, msg, data);
								self.showValide(opt, ret === true ? "" : ret, hide, ignoreNotShow);
							...
							if(ret === false || ret == "false"){
								...
								self.showValide(opt, "", hide, ignoreNotShow);
								opt.proxyCallback(false, eventName, val, msg, data);
								return false;
							}
							else {
	...
	*/
	valide: function(name, eventName, opts){
		...
		for(var i in obj){
			var opt = obj[i], ret = this.handleValide(opt, eventName, false, opt && opt.remote && opt.remote.dataType == "jsonp"? true: false);
	...
	* @param {Boolean} ignoreNotShow 是否忽略不显示
	*/
	showValide: function(opt, msg, hide, ignoreNotShow){
		Exp.alertBox({
			type:"validate",
			msg: msg,
